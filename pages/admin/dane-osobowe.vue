...
const form = reactive<PersonalDataPageContent>(createDefault())
const saving = ref(false)

// Cast to `any` to avoid Nuxt typed-route inference blowing up TS ("Excessive stack depth...")
const { data: pageData, pending, refresh } = useFetch<SitePageResponse>('/api/site-pages/dane-osobowe' as any, {
...
const save = async () => {
  ...
  try {
    await $fetch('/api/site-pages/dane-osobowe', { method: 'POST', body: { content: form } })
    await refresh()
    ...
    <section class="py-8">
      <v-container>
        <div class="d-flex flex-wrap align-center justify-space-between ga-3 mb-6">
          <div>
            <h1 class="text-h5 font-weight-bold mb-1">Dane osobowe</h1>
            <div class="text-body-2 text-medium-emphasis">Edycja treści strony /dane-osobowe</div>
          </div>
        ...
        <v-card>
          ...
          <v-divider />
          <v-card-text>
            <RichTextEditor v-model="form.body" label="Dane osobowe" />
          </v-card-text>
        </v-card>
      </v-container>
    </section>
